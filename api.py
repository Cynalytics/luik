...
from luik.clients.dependencies.plugins import PluginService, get_plugin_service
from luik.clients.scheduler_client import SchedulerAPIClient, TaskStatus
from luik.config import settings
from luik.job_handler import get_environment_settings, get_octopoes_api_connector
from luik.job_models import BoefjeMeta
from luik.models import Boefje
...
#         mime_types = _default_mime_types(
#             boefje_meta.boefje.id, boefje_meta.boefje.version
#         )
#         for file in boefje_output.files:
#             raw = base64.b64decode(file.content)
#             # when supported, also save file.name to Bytes
#             bytes_client.save_raw(
#                 task_id, raw, mime_types.union(file.tags if file.tags else [])
#             )
...
# def _default_mime_types(id_: str, version: str | None) -> set[str]:
#     mime_types = {f"boefje/{id_}"}

#     if version is not None:
#         mime_types = mime_types.union({f"boefje/{id_}-{version}"})

#     return mime_types


# def create_boefje_meta(task, plugin: Boefje) -> BoefjeMeta:
...
#     self.ctx.services.katalogus.get_plugin_settings(
